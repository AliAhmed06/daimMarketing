...
const RegisterPage = ({authProviders}) => {
  ...
  const registerHandler = async (data, e) => {
    ...
    try {
      ...
      let res = await fetch(`${process.env.NEXT_PUBLIC_DOMAIN_NAME}/api/register`, {
          headers: {
      ...
      <div className="flex items-center justify-center min-h-[800px] ">
        <div className="space-y-3 border border-second rounded-sm shadow-md w-[500px] flex flex-col items-center justify-center p-10">
          ...
          <TextField 
              ...
              <button 
                className="bg-second text-white py-3 px-5 w-full rounded-full"
              >Register</button>            
          </form>
          <p>Already have an account? <Link href="/auth/login" className="text-second hover:underline">Login</Link> </p>
