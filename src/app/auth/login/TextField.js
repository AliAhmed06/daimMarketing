...
const TextField = ({type, Icon, placeholder, register, errorMessage}) => {
  ...
  return (
    ...
    <div className='bg-gra-light h-[50px] md:w-[350px] rounded-full flex items-center px-8 gap-5 border border-gray focus-within:border-one '>
        ...
        { type === "password" && (
          showPassword ? (
            <AiFillEye className='text-gray cursor-pointer h-[40px]' onClick={() => setShowPassword(prev => !prev)} />
          ) : (
            <AiFillEyeInvisible className='text-gray cursor-pointer h-[20px]' onClick={() => setShowPassword(prev => !prev)} />
          )
        ) }
    </div>
