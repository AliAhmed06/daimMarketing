  ...
  return (
    <div className=''>      
      <nav className={`w-full fixed top-0 left-0 z-50 ${navbar ? "bg-white " : "bg-transparent " } ${videoBg == false && 'bg-white shadow-md text-black'} ${isScroll && 'bg-white shadow-md'} `}>
        {/* Home icon */}
        <Link href={"/"} title='Dayim Marketing ' className='text-second top-[30px] left-[10px] lg:left-[40px] w-[50px] h-[50px] absolute  md:block cursor-pointer'><AiFillHome size={30} /></Link>
        <div className="justify-between px-4 mx-auto lg:max-w-7xl md:items-center md:flex md:px-8">
          <div>
            <div className="flex items-center justify-between  md:block">
              {/* LOGO */}
              <Link href="/dayim-signature-apartments">
                <img src="/images/dsa/dsa_logo_golden.png" alt='' className='h-[100px] '  />
              </Link>
            ...
            >
              <ul className="h-screen md:h-auto items-center justify-center md:flex  pt-2 ">
                <li className={`${navbar ? 'text-black' : isScroll ? 'text-black' : videoBg == false ? 'text-black' : 'text-white' } pb-6  uppercase text-md font-semibold hover:underline ${pathname == "/about" && "underline"} py-2 md:px-6 text-center border-b-2 md:border-b-0 hover:bg-purple-900  border-purple-900  md:hover:text-purple-600 md:hover:bg-transparent`}>
                  <Link href="/dayim-signature-apartments/about" onClick={() => setNavbar(!navbar)}>
                    About
                  </Link>
                </li>
                ...
                <li className={`${navbar ? 'text-black' : isScroll ? 'text-black' : videoBg == false ? 'text-black' : 'text-white' } pb-6 uppercase text-md font-semibold hover:underline ${pathname == "/vr" && "underline"} py-2 md:px-6 text-center border-b-2 md:border-b-0  hover:bg-purple-900  border-purple-900  md:hover:text-purple-600 md:hover:bg-transparent`}>
                  <Link href="/dayim-signature-apartments/vr" onClick={() => setNavbar(!navbar)}>
                    VR
                  </Link>
                </li>
                <li className={`${navbar ? 'text-black' : isScroll ? 'text-black' : videoBg == false ? 'text-black' : 'text-white' } pb-6 uppercase text-md font-semibold hover:underline ${pathname == "/experiences" && "underline"} py-2 md:px-6 text-center border-b-2 md:border-b-0  hover:bg-purple-900  border-purple-900  md:hover:text-purple-600 md:hover:bg-transparent`}>
                  <Link href="/dayim-signature-apartments/experiences" onClick={() => setNavbar(!navbar)}>
                    Experiences
                  </Link>
                </li>
                <li className={`${navbar ? 'text-black' : isScroll ? 'text-black' : videoBg == false ? 'text-black' : 'text-white' } pb-6 uppercase text-md font-semibold hover:underline ${pathname == "/experiences" && "underline"} py-2 md:px-6 text-center border-b-2 md:border-b-0  hover:bg-purple-900  border-purple-900  md:hover:text-purple-600 md:hover:bg-transparent`}>
